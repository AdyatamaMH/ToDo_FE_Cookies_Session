...
function App() {
  ...
  const markDone = async (id, completed) => {
    try {
      const token = await auth.currentUser.getIdToken(true); 
      console.log(`Sending request to toggle task ${id} to ${completed}`);
      await axios.put(`http://localhost:8000/tasks/toggle/${id}`, {
        status: completed 
      }, {
        headers: {
          Authorization: `Bearer ${token}`
      ...
      console.log(`Task ${id} status toggled to ${completed}`);
